...
- `std::promise<T>` provides a means of setting a value of type T that can later be read through an associated std::future<T> object.
  ...
  - `std::promise<T>` can be retrieved only once.
  - `get_future()` member function to get the std::future. `set_value()` member function to set the value and get the future ready.
...
- `std::future` is modeling unique ownership of a asynchronous result. `std::shared_future` is modeling shared ownership of an asynchronous result, which is copyable.
...
- Focus of operations, forget mechanics.
  ...
  - `latch` and `barrier`
    ...
    - `latch` is a synchronization object that becomes ready when its counter is decremented to zero.
      A lightweight facility for waiting for a series of events to occur 
    - `barrier` is a reusable synchronization component used for internal synchronization between a set of threads.
      - `latch` allows one single thread to decrement the counter multiple times
        `barrier` allows one single thread to decrement the counter only once. 
      - `latch` is one shot device,
        `barrier` is reusable.
    - When multiple thread processing a chunk of data, use a `latch` to ensure its completion.
